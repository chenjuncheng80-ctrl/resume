...
const landY = JSON.parse(await evalx(`JSON.stringify({ aboutTop: document.querySelector('#about').getBoundingClientRect().top, landY: window.__skillFall._landY(), innerH: window.innerHeight, expected: Math.min(document.querySelector('#about').getBoundingClientRect().top + 34, window.innerHeight - 96) })`));
console.log('landY', JSON.stringify(landY));
